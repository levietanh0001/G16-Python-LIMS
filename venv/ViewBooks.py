...
def viewBooks():
    ...
    trv.column('#3', width=150, minwidth=100)
    trv.column('#4', width=50, minwidth=75)
    ...
    query = "select book_id, book_title, authors.author_name, available_copies from books, authors where books.author_id = authors.author_id"
    cur.execute(query)
    rows = cur.fetchall()
